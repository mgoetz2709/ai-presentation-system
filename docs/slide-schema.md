...
Every phase in the pipeline (Nova → Luca → Veronika → Jürgen) works on the *same* slide
...
assigned once, by Nova, and never changes downstream.**

## The rule

1. **Nova assigns `slide_id`s** (1, 2, 3, … in final presentation order) when she delivers the
   Storyline Blueprint. This is the only point where slide count is decided.
...
3. If an agent believes a slide should be added, split, or removed (e.g. Luca finds a slide
   needs 8 lines and must become two slides), they **flag it to Maximilian** with the proposed
   change instead of doing it silently. Maximilian either approves it (and tells Nova to reissue
   the index with a note of what changed and why) or pushes back. The index is never forked.
...
```markdown
| slide_id | working headline | slide role in arc | type (assigned by Veronika, blank before Phase 5) |
|---|---|---|---|
...
| Phase | Adds |
|---|---|
| **Nova** (Storyline) | `working_headline`, `core_message`, `role_in_arc`, `framework` (deck-level, not per-slide) |
| **Luca** (Content) | `final_headline` (assertion, may refine Nova's), `bullets[]` (≤5), `speaker_notes`, `visual_spec` (what data/diagram this slide needs), `alt_text` |
| **Veronika** (Design) | `slide_type` (TITLE/CONTENT/STAT/CHART/PROCESS/COMPARISON/TIMELINE/CLOSING), `chart_script_ref` (if CHART), `layout_notes` |
| **Jürgen** (Quality) | `dimension_findings[]` (dimension, severity, description, correction, responsible agent) |
...
When Maximilian runs a presentation project, the four documents (Research Brief, Storyline
